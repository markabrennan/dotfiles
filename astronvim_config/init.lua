return {
	...
	lsp = {
		servers = {
			"pyright", -- Explicitly enable Pyright, even though it's working already
			"ruff_lsp", -- Add Ruff LSP
		},
		formatting = {
			format_on_save = {
				enabled = true, -- Enable formatting on save globally
				allow_filetypes = { "python" },
			},
			disabled = { "pyright" }, -- Disable Pyright's formatting to avoid conflicts
		},
	},
	...
	polish = function()
		...
		vim.api.nvim_create_user_command("RuffDiff", function()
			...
			vim.cmd("vsplit | terminal ruff format --diff" .. file)
		...
		lspconfig.ruff_lsp.setup({
			on_attach = function(client, bufnr)
				...
				for _, other_client in ipairs(vim.lsp.get_active_clients()) do
